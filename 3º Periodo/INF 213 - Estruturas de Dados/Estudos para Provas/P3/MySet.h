...
// 	Node<T> * left, * right; //filhos da esquerda e direita de cada nodo.
// 	Node<T> * parent; //pai de cada nodo
// 	Node<T> * neighbor; // Vizinho a direita de um nodo. Isto é, o nodo que está ao seu lado no mesmo nível.
// };
...
public:
	...
	int sumLowerThan(int n);
	void sumLowerThan(Node<T> * node,int n, int *k);
	int totalMissing();
	void totalMissing(Node<T> * nodo, int *k, int *l);
	int getMediana();
	int altura(Node<T> * raiz);
	void preencheFBA();
	void preencheFB(Node<T> * raiz);
	void setNeighbors();

...
public:
	...
	bool empty() ;
	int size() ; /*etapa 2: implemente esta funcao sem usar o membro de dados dataSize*/  // na STL List, a funcao size() calcula o tamanho da lista dinamicamente (exercicio: qual a ordem de complexidade?)
...
public:
	...
	MyList2Iterator(Node<T> *ptr_, Node<T> **prtDataLast_): ptr(ptr_), prtDataLast(prtDataLast_) {}
...
int MyList2<T>::size() {
	...
	for (iterator itAux = begin(); itAux != end(); itAux++) {
		qntdElem++;
	}

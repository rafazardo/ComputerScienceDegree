...
void MyHashMap<KEY,VALUE,HASH>::set(const  KEY &k, const VALUE &v) {
	...
	std::pair<KEY,VALUE> elemento(k,v); 
...
void MyHashMap<KEY,VALUE,HASH>::reHash(int newSize) {
	...
	MyVec<MyList2<std::pair<KEY,VALUE> > > tableAux = table;
	...
	for(int i=0;i<tableAux.size();i++) {
		for(typename MyList2<std::pair<KEY,VALUE> >::iterator it = tableAux[i].begin();it!=tableAux[i].end();it++) {
